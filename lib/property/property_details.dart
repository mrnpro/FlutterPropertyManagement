...
class PropertyDetailsState extends State<PropertyDetails> {
  ...
  Widget build(BuildContext context) {
    final propertySellDate = getDateFromDateTimeInSpecificFormat(
        dateFormat, widget.sellModel.updatedAt ?? "yyyy-MM-dd");
  ...
  Widget _buildSliverAppBarWidget() {
    return SliverAppBar(
      ...
      flexibleSpace: FlexibleSpaceBar(
        ...
        background: Hero(
          tag: widget.sellModel.id ?? "selected",
          child: widget.sellModel.sellImages != null &&
                  (widget.sellModel.sellImages?.length ?? 0) > 0
              ? Stack(
                  children: <Widget>[
                    PageView.builder(
                      onPageChanged: (view) {
    ...
    return SliverList(
      delegate: SliverChildListDelegate([
        Container(
          ...
          child: Text(
            getPropertyTypeById(widget.sellModel.sellType ?? 0),
        ...
        Container(
          ...
          width: double.infinity,
          child: Text(
            (widget.sellModel.sellDescription?.isEmpty ?? false)
                ? "-"
