    ...
    return Hero(
      ...
      child: Container(
        ...
        child: sellModel.sellImages == null ||
            ...
            : fetchImageFromNetworkFileWithPlaceHolder(
                sellModel.sellImages![0]),
        /*CachedNetworkImage(
        ...
        Column(
          ...
          children: <Widget>[
            ...
            Padding(
                padding: const EdgeInsets.only(top: 5.0, right: 5.0, left: 5.0),
                child: Text(
                    "${sellModel.sellBedrooms} BHK ${getPropertyTypeById(sellModel.sellType ?? 0)}")),
            Padding(
              padding: const EdgeInsets.only(top: 5.0, right: 5.0, left: 5.0),
  ...
  fetchSellPropertyData() async {
    ...
    try {
      final propertySellReference =
          FirebaseDatabase.instance.ref().child("Property").child("Sell");

      propertySellReference.onValue.listen((DatabaseEvent event) {
        propertySellList = [];
        if (event.snapshot.value != null) {
          print("value: ${event.snapshot}");
          print("value: ${event.snapshot.key}");
          print("value: ${event.snapshot.value}");
          final encodedValue = jsonEncode(event.snapshot.value);
          final decodedValue = jsonDecode(encodedValue);

          for (var value in decodedValue) {
